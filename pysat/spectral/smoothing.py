...
def boxcar(y, window_size=3):
    ...
    return Series(np.convolve(np.asarray(y), filt, mode='same'), index=y.index)
...
def gaussian(y, window_size=3, sigma=2):
    ...
    return Series(signal.convolve(np.asarray(y), filt, mode='same'), index=y.index)

methods = {'boxcar': boxcar,
           'gaussian': gaussian}

def smooth(y, method='boxcar', **kwargs):
